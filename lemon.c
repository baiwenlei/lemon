...
{
  static char templatename[] = "./lempar.st";
  char buf[1000];
  FILE *in;
  char *tpltname;
  char *cp;
  ...
  if( cp ){
    lemon_sprintf(buf,"%.*s.st",(int)(cp-lemp->filename),lemp->filename);
  }else{
    lemon_sprintf(buf,"%s.st",lemp->filename);
  }
  if( access(buf,004)==0 ){
    tpltname = buf;
...
void ReportHeader(struct lemon *lemp)
{
  ...
  in = file_open(lemp, ".ht", "rb");
  if (!in) {
    fprintf(stderr,"Can't open header template file \"%s\".\n", lemp->outname);
    return;
  }

  out = file_open(lemp,".h","wb");
  if(!out){
    fprintf(stderr,"Can't open or write header file \"%s\".\n", lemp->outname);
    fclose(in);
    return;
  }
